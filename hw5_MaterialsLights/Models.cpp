...
void Models::load(GLint v, GLint c, GLint n)
{
    vPosition = v;
    vColor = c;
    ...
    glBufferData(GL_ARRAY_BUFFER, 4*(lon+1)*(lat+1)*sizeof(GLfloat) + 4*(lon+1)*(lat+1)*sizeof(GLfloat) + 3*(lon+1)*(lat+1)*sizeof(GLfloat), NULL, GL_DYNAMIC_DRAW);

    glBufferSubData(GL_ARRAY_BUFFER, 0, 4*(lon+1)*(lat+1)*sizeof(GLfloat), points);
    glBufferSubData(GL_ARRAY_BUFFER, 4*(lon+1)*(lat+1)*sizeof(GLfloat), 4*(lon+1)*(lat+1)*sizeof(GLfloat), colors);
    glBufferSubData(GL_ARRAY_BUFFER, 4*(lon+1)*(lat+1)*sizeof(GLfloat) + 4*(lon+1)*(lat+1)*sizeof(GLfloat), 3*(lon+1)*(lat+1)*sizeof(GLfloat), normalvectors);

    glVertexAttribPointer(vNormal, 3, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(4*(lon+1)*(lat+1)*sizeof(GLfloat) + 4*(lon+1)*(lat+1)*sizeof(GLfloat)));
    ...
    glBufferSubData(GL_ARRAY_BUFFER, 0, 4*(lon+1)*(lat+1)*sizeof(GLfloat), points);
    glBufferSubData(GL_ARRAY_BUFFER, 4*(lon+1)*(lat+1)*sizeof(GLfloat), 4*(lon+1)*(lat+1)*sizeof(GLfloat), border_colors);
    glBufferSubData(GL_ARRAY_BUFFER, 4*(lon+1)*(lat+1)*sizeof(GLfloat) + 4*(lon+1)*(lat+1)*sizeof(GLfloat), 3*(lon+1)*(lat+1)*sizeof(GLfloat), normalvectors);

    glVertexAttribPointer(vNormal, 3, GL_FLOAT, GL_TRUE, 0, BUFFER_OFFSET(4*(lon+1)*(lat+1)*sizeof(GLfloat) + 4*(lon+1)*(lat+1)*sizeof(GLfloat)));
    glVertexAttribPointer(vColor, 4, GL_FLOAT, GL_TRUE, 0, BUFFER_OFFSET(4*(lon+1)*(lat+1)*sizeof(GLfloat)));
    glVertexAttribPointer(vPosition, 4, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(0));
...
void Models::createCylinderOBJ(GLfloat r1, GLfloat r2, GLfloat len, GLint ilon, GLint ilat)
{
    ...
    for (int i = 0; i <= lon; i++)
        for (int j = 0; j <= lat; j++)
        {
            ...
            x1 = (r2-r1)/len*sin(Theta);
            y1 = (r2-r1)/len*cos(Theta);
            z1 = 1;
